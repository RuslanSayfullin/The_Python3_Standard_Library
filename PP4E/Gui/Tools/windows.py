...
class _window:
    ...
    def configBorders(self, app, kind, iconfile):
        if not iconfile:                # ярлык не был передан?
            iconfile = self.findIcon()  # поиск в тек. каталоге и в каталоге модуля
        title = app
        if kind: title += ' - ' + kind
...
class MainWindow(Tk, _window):
    ...
    def okayToQuit(self):
        return True     # переопределить, если используются потоки выполнения
...
class PopupWindow(Toplevel, _window):
    ...
    def quit(self):                                         # переопределить, если потребуется изменить
        if askyesno(self.__app, 'Verify Quit Window?'):     # или вызвать destroy чтобы закрыть окно
            self.destroy()
