...
def threadChecker(widget, delayMsecs=100, perEvent=1):  # 10 раз/сек, 1/таймер
    for i in range(perEvent):                           # передайте другие значения,
...
class ThreadCounter:
    ...
    def decr(self):
        ...
        self.mutex.release()
    def __len__(self):
        return self.count   # True/False, если используется, как флаг
...
if __name__ == '__main__':      # самотестирование при запуске в виде сценария
    ...
    def threadaction(id, reps, progress): # то, что делает поток
        for i in range(reps):
            time.sleep(1)
            if progress: progress(i)        # обработчик progress: в очередь, ошибочный номер: неудача
    ...
    text.bind('<Button-1>',     # в 3.x функция list необходима для получения
